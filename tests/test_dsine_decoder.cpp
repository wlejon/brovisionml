// DSINE v02 decoder: real-weights golden parity test.
//
...
// bytes, runs dsine::preprocess, runs the encoder (committed) to produce the
// three taps, builds the uv conditioning maps from the golden intrinsics
// (remember: the model adds +0.5 to cx/cy, done inside build_uv), runs the
// decoder, and compares the initial normal map against the golden `dec_normal`
// record (3, padH/8, padW/8).
//
// The decoder rides on top of the deep conv+BN encoder stack, so the bound is
// ~2e-3 max-abs. A wildly larger diff points at a real bug (WS standardization,
// GroupNorm eps, align_corners on the upsample, the +0.5, or ReLU/LeakyReLU)
...
// Reads the BVMLDSN1 golden up to and including the dec_normal record (seeking
// past the three encoder taps). The final full-res map is ignored.
struct Golden {
...
bool load_golden(const std::string& path, Golden& g) {
    ...
    // dec_normal record.
    if (!read_one(f, g.dn_C) || !read_one(f, g.dn_H) || !read_one(f, g.dn_W))
        return false;
    const std::size_t dn_n =
        static_cast<std::size_t>(g.dn_C) * g.dn_H * g.dn_W;
    if (!read_vec(f, g.dec_normal, dn_n)) return false;
    return true;
}
...
void run_case(const std::string& dir, const std::string& path) {
    ...
    std::printf("    dec_normal: got (%d,%d,%d)  golden (%d,%d,%d)\n",
                3, out.h8, out.w8, g.dn_C, g.dn_H, g.dn_W);
    CHECK(g.dn_C == 3 && out.h8 == g.dn_H && out.w8 == g.dn_W);

    const std::size_t n =
        static_cast<std::size_t>(g.dn_C) * g.dn_H * g.dn_W;
    if (static_cast<std::size_t>(out.normal.cols) != n) {
        std::printf("    FAIL  dec_normal element count %d != golden %zu\n",
                    out.normal.cols, n);
        ++g_failures;
        return;
    }
    double mx = 0.0, mn = 0.0;
    diff_stats(out.normal.host_f32(), g.dec_normal.data(), n, mx, mn);
    std::printf("    dec_normal: max-abs=%.3e  mean-abs=%.3e\n", mx, mn);
    CHECK(mx < 2e-3);
}
...
int main() {
    ...
    if (g_failures == 0) {
        std::printf("  OK  all dsine_decoder parity checks passed\n");
        return 0;
    }
