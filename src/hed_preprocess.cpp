...
PreprocessedImage preprocess(const uint8_t* rgb, int w, int h, int channels,
    ...
    if (!rgb || w <= 0 || h <= 0)
        throw std::runtime_error("hed::preprocess: zero-size or null image");
    if (channels != 1 && channels != 3 && channels != 4)
        throw std::runtime_error("hed::preprocess: channels must be 1, 3, or 4");
    ...
    hed_proc_dims(w, h, detect_resolution, proc_w, proc_h);
    ...
    // 3. Pack to planar NCHW FP32, leaving values in [0,255] (the scale the
    //    model's `norm` bias is defined against — no normalize here).
