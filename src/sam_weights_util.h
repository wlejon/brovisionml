...
// Shared private weight-loading helpers for the SAM modules (prompt encoder,
// mask decoder). NOT a public header — it lives in src/ and is included only by
// the SAM .cpp files. It mirrors the small loader the image encoder carries in
// its own translation unit: every weight is widened to host FP32 regardless of
// the checkpoint's on-disk dtype (F32 / F16 / BF16), so a module loads on the
// host and migrates with to(Device) afterwards.
...
namespace brovisionml::sam::detail {
...
// element offset lets a combined block (e.g. a packed qkv) be sliced without a
// second copy. `who` is the caller's error prefix (e.g. "sam::MaskDecoder: ").
